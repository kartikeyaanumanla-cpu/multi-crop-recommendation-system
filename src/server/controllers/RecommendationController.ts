...
export class RecommendationController {
  ...
  public handleSuggestion = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const validatedData = SuggestionRequestSchema.parse(req.body);
      const suggestions = await this.recommendationService.suggestCrops(validatedData);
   ...
   */
  public handleRecommendation = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const recommendation = await this.recommendationService.recommendCrops(validatedData);
