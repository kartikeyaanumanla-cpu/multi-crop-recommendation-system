    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-[#f8fcf9] to-teal-100/40 p-6 md:p-12 relative overflow-hidden flex items-center justify-center font-sans">
      ...
      >
        <div className="bg-white/70 backdrop-blur-xl p-8 md:p-10 rounded-[32px] shadow-2xl shadow-emerald-900/5 border border-white/60 text-center">
          ...
          <h1 className="text-3xl font-extrabold text-emerald-950 tracking-tight mb-2 drop-shadow-sm">Create Account</h1>
          <p className="text-emerald-800/70 font-medium mb-8">Join to generate optimized farm strategies</p>
          ...
          <form onSubmit={handleSignup} className="space-y-5 text-left">
            ...
            <div>
              <label className="block text-xs font-bold text-emerald-800/60 uppercase tracking-widest mb-1.5 ml-1">Full Name</label>
              <div className="relative">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-emerald-600/50" />
                <input
                  type="text"
                  value={name}
              ...
              <div className="w-full rounded-full border border-white/20 py-3 px-8 flex items-center justify-center gap-3 relative overflow-hidden">
                {isLoading ? (
                  <>
                    ...
                    <span className="font-bold tracking-wide">Creating Account...</span>
                  </>
                ) : (
                  <>
          ...
          <p className="mt-8 text-sm text-emerald-800/60 font-medium">
            Already have an account? <Link to="/login" className="text-emerald-600 font-bold hover:underline decoration-emerald-600/30 underline-offset-4">Log in</Link>
          </p>
        </div>
      </motion.div>
    </div>
